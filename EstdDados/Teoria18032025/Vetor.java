...
class Vetor {
    Produto[] A; // Criando uma lista do tipo Gamer
    int capacity; // capacidade do vetor - length
    int size; // elementos no vetor - qtde elementos
    
    public Vetor(int capacity) {
        A = new Produto[capacity];    // Vetor de Gamers definindo a capacidade (10)
        this.size = 0;
    ...
    // Aqui tenque mudar bastante 
    // Meu objetivo é buscar o codigo do produto
    public int search(int n) {
        // busca o valor n na lista - retornando a posição ou -1, se não existir
        for (int i=0;i<size;i++)
            if (A[i].codigo == n)   // Comparando nao o objeto todo e sim só o código com n
                return i;
        return -1;
    }
